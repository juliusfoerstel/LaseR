...
first.analysis <- function(matrix.element = "Ca", output.filename = "results", subtract.bg = T, dismiss.beginning = 10, dismiss.after = 10,step = 500, exp.bg = F){
  if(grepl(pattern = "Linux", x = Sys.info()[1])){
    library(tcltk)
    files <- tk_choose.files(filters = matrix(c("CSV-Files",".csv"), nrow = 1, ncol = 2),caption = "Select all raw data files",multi = T) #choose the files to evaluate. the prompt is not nice, but it works on all systems (I hope)
  } else {
    files <- choose.files(caption = "Select all raw data files",multi = T,default = "rawdata.csv")
  }
  ...
  mubg <- id.meas.and.bg(d = data[,column],
                         dismiss.beginning = dismiss.beginning ,
                         dismiss.after = dismiss.after,
                         step = step)
